...
spark.sparkContext.addPyFile("s3://ragam-landingzone/jarfiles/delta-core_2.12-0.8.0.jar")
from delta import *
class Configure:
    ...
    def __init__(self):
        ...
        self.active_destination = self.jsonData['masked-Actives']['destination']['data-location']
        self.active_format = self.jsonData['masked-Actives']['destination']['file-format']
...
class Transform(Configure):
    ...
    def scd2_implementaion(self, df, lookup_location, pii_cols,dataset): 
        file_name = 'Actives'
...
for dataset in confObj.ingest_datasets:
    if dataset == "Actives":
        ...
        #scd2 implementation
        lookup_data = transObj.scd2_implementaion( actives_stagingzone_data,confObj.lookup_location,confObj.pii_cols,confObj.ingest_datasets)
        # write actives Transformed data to Staging Zone
        #transObj.raw_to_staging_actives(lookup_data)
        transObj.to_write_data(lookup_data,confObj.active_destination,confObj.active_format,confObj.active_partition_columns)
